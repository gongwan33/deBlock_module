...
void CheckAvailabilityOfNeighbors()
{
  ...
  {
    currMB->mbAddrA = mb_nr - 1;								//++ 当前宏块左边的宏块
    currMB->mbAddrB = mb_nr - dec_picture->PicWidthInMbs;		//++ 当前宏块上边的宏块
    currMB->mbAddrC = mb_nr - dec_picture->PicWidthInMbs + 1;	//++ 当前宏块右上的宏块
    currMB->mbAddrD = mb_nr - dec_picture->PicWidthInMbs - 1;	//++ 当前宏块左上的宏块
...
//++ 参见6.4.9	Derivation process for neighbouring locations
...
//++ 参见6.4.9.2	Specification for neighbouring locations in MBAFF frames
...
void getLuma4x4Neighbour_Simp (int curr_mb_nr, int block_x, int block_y, int rel_x, int rel_y, PixelPos *pix)
{
  int x = 4* block_x + rel_x;
  int y = 4* block_y + rel_y;
