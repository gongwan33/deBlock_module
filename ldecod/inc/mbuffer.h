...
typedef struct storable_picture
{
  ...
  unsigned    frame_num;
  int         pic_num;
  int64       ref_pic_num        [MAX_NUM_SLICES][6][MAX_LIST_SIZE];	//++ 参考图像的图像号
  ...
  int  ***    ref_idx;       //!< reference picture   [list][subblock_x][subblock_y]	//++ 参考图像的在参考图像列表中的索引，即在列表中的位置
...
//  byte *      mb_field;      //!< field macroblock indicator
  ...
  int  ***    ref_idx;       //!< reference picture   [list][subblock_x][subblock_y]	//++ 参考图像的在参考图像列表中的索引，即在列表中的位置
  int64 ***    ref_pic_id;    //!< reference picture identifier [list][subblock_x][subblock_y]	//++ 参考图像的图像号
  int  ****   mv;            //!< motion vector       [list][subblock_x][subblock_y][component]  
  byte **     moving_block;
